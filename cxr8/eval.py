...
from X_RAY.cxr8.utils import model_name, select_model
...
def compute_stats(gt, pred):
    ...
    # Hamming_loss = hamming_loss(gt_np, pred_np)
    ...
    return AUROCs, roc_curves, mean_TPR, mean_TNR, mean_PPV, PPV_dict, TNR_dict, TPR_dict
...
def eval_function(args, model):
    ...
    for i, (inputs, target, weight) in tqdm(enumerate(dataloader), total=int(test_length / args.batch_size)):
        ...
        with torch.no_grad():
            ...
            output = model(inputs)
            # TODO try removing the sigmoid and try
            # output = torch.sigmoid(output)
        pred = torch.cat((pred, output.data), 0)


    AUROCs, roc_curves, mean_TPR, mean_TNR, mean_PPV, PPV_dict, TNR_dict, TPR_dict = compute_stats(gt, pred)
...
if __name__ == '__main__':
    ...
    pathModel = os.path.join(current_location, "savedModels",
                             "densenet121_AVG_MAX10_IMG_SIZE_512num_class_8_best_model.pth")
